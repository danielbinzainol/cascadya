...
class RteGenerationForecastApiError(RuntimeError):
...
class RteGenerationForecastAuthError(RteGenerationForecastApiError):
    pass


class RteGenerationForecastForbiddenError(RteGenerationForecastApiError):
    pass


class RteGenerationForecastNotFoundError(RteGenerationForecastApiError):
    pass


class RteGenerationForecastBadRequestError(RteGenerationForecastApiError):
    pass


class RteGenerationForecastRateLimitError(RteGenerationForecastApiError):
    pass


@dataclass(slots=True)
class RteGenerationForecastAuthConfig:
...
class RteGenerationForecastClient:
    def __init__(
        self,
        *,
    ...
    async def __aenter__(self) -> RteGenerationForecastClient:
        return self
    ...
    async def _resolve_access_token(self) -> str:
        ...
        if not has_basic_auth_b64 and not has_client_credentials:
            raise RteGenerationForecastAuthError(
    ...
    def _raise_for_status(self, response: httpx.Response) -> None:
        ...
        try:
            ...
            if parsed_payload and parsed_payload.error_description:
                ...
                message = f"RTE Generation Forecast API error {status_code}"
        except ValueError:
            payload = response.text
            message = payload if isinstance(payload, str) and payload else f"RTE Generation Forecast API error {status_code}"

        if status_code == 400:
            raise RteGenerationForecastBadRequestError(message, status_code=status_code, payload=payload)
        if status_code == 401:
            raise RteGenerationForecastAuthError(message, status_code=status_code, payload=payload)
        if status_code == 403:
            raise RteGenerationForecastForbiddenError(message, status_code=status_code, payload=payload)
        if status_code == 404:
            raise RteGenerationForecastNotFoundError(message, status_code=status_code, payload=payload)
        if status_code == 429:
            raise RteGenerationForecastRateLimitError(message, status_code=status_code, payload=payload)
        raise RteGenerationForecastApiError(message, status_code=status_code, payload=payload)
