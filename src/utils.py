...
COEFF_M3_TO_NM3 = (
    1.2  # dépendent de pression et température du gaz, ceux-là sont pour Tarkett.
)
...
def convert_gas_units(
    ...
    df = df.copy()
    df[converted_value_col] = (
        df[value_col] * conversion_scales[target_unit] / conversion_scales[initial_unit]
    )
...
def convert_saturated_steam_units(
    ...
    if initial_unit not in ACCEPTED_SATURATED_STEAM_UNITS:
        raise ValueError(
            f"initial_unit {initial_unit} not in {ACCEPTED_SATURATED_STEAM_UNITS}"
        )
    if target_unit not in ACCEPTED_SATURATED_STEAM_UNITS:
        raise ValueError(
            f"target_unit {target_unit} not in {ACCEPTED_SATURATED_STEAM_UNITS}"
        )
    if initial_unit == target_unit:
        raise ValueError("initial_unit and target_unit are identical")
