...
3. Faire des modèles de régressions linéaires 
	...
	2. Faire des modèles linéaires sur la base des composantes combinées, et les évaluer
		1. Créer les modèles pour les composantes composées : créer des modèles "imbriqués":
			...
			2. Modèle 1: ajouter l'une des composantes individuelles seulement (parmi month, weekday, hour, minute, holiday).  
...
5. inférence des paramètres (explication/diagnostique du poids de chacun) : sur le modèle retenu, c'est l'analyse finale, pas l'outil de sélection
	...
	2. Vérifier a posteriori des régressions linéaires si les résidus suivent une loi normale, ce qui donnera de la confiance dans les résultats du modèle. Cela est testé avec :
	...
	2. Etudier l'impact des composantes individuelles
		1. Vérifier si les conditions permettent d'utiliser un test paramétrique de Welch ANOVA : 
			...
			4. Est-ce qu'il y a assez de données par groupes ? (indication (pas absolu) n >= 30)
...
6. réaliser des sanity checks avec agregations à 30 minutes ou 1 heure.
	1. Rejouer la pipeline complète ci-dessus avec des agrégations à 30min ou 1h.
	2. Vérifier la stabilité du classement des modèles et du signe des effets.
	3. Un sanity check business, c'est aussi se demander : "est ce que les prédictions sont alignées avec la connaissance métier/business" (regarder les pics, jours férier, weekend, contraintes opérationnelles)
