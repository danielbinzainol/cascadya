...
def test_copy_median_values_extends_one_day_with_overall_median_when_no_grouping() -> (
    None
):
    df = pd.DataFrame(
        {
            "timestamp_col": pd.date_range(
                "2026-01-01 00:00:00", periods=4, freq="1h", tz="UTC"
            ),
    ...
    assert new_rows["timestamp_col"].iloc[0] == pd.Timestamp(
        "2026-01-01 04:00:00", tz="UTC"
    )
