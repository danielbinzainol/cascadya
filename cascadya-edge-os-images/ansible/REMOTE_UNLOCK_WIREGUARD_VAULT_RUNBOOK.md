...
## 0. Fresh IPC boot and manual-rerun checks

Before re-running the workflow on an IPC that has just booted, verify the local
state first:
...
If `/data` is not mounted but the host was already prepared for remote unlock,
...
If you are re-running playbooks manually from the control-panel VM instead of
through the FastAPI runner, reload the generated inventory and vars explicitly:
...
If one of these variables is empty, Ansible may start parsing the whole
`/opt/control-panel/control_plane/auth_prototype` tree as inventory and emit a
large warning storm ending with `No inventory was parsed`.
