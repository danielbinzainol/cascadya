...
def plot_weekday_seasonal(df: pd.DataFrame, value_col, option_mean=False):
    if option_mean:
...
def detect_plot_trend(df, obs_in_window):
    moving_average = df.rolling(
        ...
        min_periods=np.floor(
            obs_in_window / 2, casting="unsafe", dtype=int
        ),  # choose about half the window size
    ...
    moving_average.plot(
        ax=ax,
        linewidth=3,
        title=f"France champignon Conso - {obs_in_window} obs for Moving Average",
        legend=False,
    )
    plt.tight_layout()
    plt.show()
...
def plot_gap_filled_timeseries(
    ...
    for tag, color in tag_colors.items():
        ...
        if subset.empty:
            continue
        x_subset = (
            subset[timestamp_col] if timestamp_col in df.columns else subset.index
        )
        ax.scatter(
            x_subset,
            subset[value_col],
