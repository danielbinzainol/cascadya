    ...
    match (end, respect_time, respect_weekdays):
        case ("2026-01-07", False, False):
            ...
            new_ts = pd.date_range(
                "2026-01-26T00:00:00", "2026-01-26T12:00:00", freq="15min", tz="UTC"
            )
        case ("2026-01-07", True, False):  # will raise a warning, intended
            to_copy = np.linspace(144, 192, 49)
            new_ts = pd.date_range(
                "2026-01-26T12:00:00", "2026-01-27T00:00:00", freq="15min", tz="UTC"
            )
        case ("2026-01-07", True, True):  # will raise a warning, intended
            to_copy = np.linspace(144, 192, 49)
            new_ts = pd.date_range(
                "2026-01-27T12:00:00", "2026-01-28T00:00:00", freq="15min", tz="UTC"
            )
        case ("2026-01-11", False, False):
            to_copy = np.linspace(144, 576, 433)
            new_ts = pd.date_range(
                "2026-01-26T00:00:00", "2026-01-30T12:00:00", freq="15min", tz="UTC"
            )
        case ("2026-01-11", True, False):  # will raise a warning, intended
            to_copy = np.linspace(144, 576, 433)
            new_ts = pd.date_range(
                "2026-01-26T12:00:00", "2026-01-31T00:00:00", freq="15min", tz="UTC"
            )
        case ("2026-01-11", True, True):  # will raise a warning, intended
            to_copy = np.linspace(144, 576, 433)
            new_ts = pd.date_range(
                "2026-01-27T12:00:00", "2026-02-01T00:00:00", freq="15min", tz="UTC"
            )
        case _:
            raise ValueError(
                f"Unknown case: ({end}, {respect_time}, {respect_weekdays})"
            )

    return pd.DataFrame({"timestamp_col": new_ts, "value_col": to_copy})


def expected_res_median(
    respect_holidays, respect_weekdays, respect_time, extension
):  # the correct values are obtained in the excel file "comprendre_test_median.xlsx"
    match (respect_holidays, respect_weekdays, respect_time, extension):
        ...
        case _:
            raise ValueError(
                f"Unknown case: ({respect_holidays}, {respect_weekdays}, {respect_time}, {extension})"
            )
...
def y():
    ...
    ts = pd.date_range(
        start="2026-01-05", end="2026-01-26", freq="15min", inclusive="left", tz="UTC"
    )
    val = list(
        range(len(ts))
    )  # from 0 to 21*24*4 - 1 = 2015 included #end is not included
...
def test_copy_median_values(
    y, respect_holidays, respect_weekdays, respect_time, extension
):
    # 1. initialise test data
    expected_result = expected_res_median(
        respect_holidays, respect_weekdays, respect_time, extension
    )

    # 2. call function to test
    y_extended = copy_median_values(
        y,
        "timestamp_col",
        "value_col",
    ...
    pd.testing.assert_series_equal(
        result, expected_result, check_index=False, check_names=False
    )
