...
def simple_copy(
    ...
    ):  # todo check that a missing tzinfo does raise the error # todo check that a pd.na falls here
        raise ValueError(
            f"Missing or wrong tzinfo for start (expected to be in UTC): {start}"
        )
    if end.tzinfo != datetime.timezone.utc:
        raise ValueError(
            f"Missing or wrong tzinfo for end (expected to be in UTC): {end}"
        )
    ...
    if not df[timestamp_col].eq(start).any():  # correct test for tz-aware data
        ...
        raise ValueError(
            f"start {start} must be present in {timestamp_col}, spanning {df[timestamp_col].min(), df[timestamp_col].max()}"
        )
    if not df[timestamp_col].eq(end).any():
        raise ValueError(
            f"end {end} must be present in {timestamp_col}, spanning {df[timestamp_col].min(), df[timestamp_col].max()}"
        )
    elapsed = df[timestamp_col].diff()
    if elapsed.value_counts().size != 1:
        raise ValueError(
            f"The elapsed time in {timestamp_col} is inconsistent, should be nomalized."
        )
    ...
    if respect_weekdays:
        if not respect_time:
            warnings.warn(
                "Option respect_weekdays is True and overrides option respect_time. Option respect_time was chosen as False, but changed to True."
            )
        respect_time = False
    ...
    if respect_time:
        if start_time >= expected_next_timestamp_time:
            ...
            start_new = pd.Timestamp.combine(
                expected_next_timestamp_date + pd.Timedelta(days=1), start_time
            )
        ...
        if start_weekday == expected_next_timestamp_weekday:
            ...
            days_delta = (
                start_weekday - expected_next_timestamp_weekday
            ) % 7  # for a week
    ...
    if start_new != expected_next_timestamp:
        warnings.warn(
            f"start ({start}) might be incorrectly chosen, it leaves a gap at the end of the extended dataframe. Yields {start_new} instead of {expected_next_timestamp}."
        )
...
def copy_median_values(
    ...
    if group_cols:
        medians = (
            tmp.groupby(group_cols, dropna=False)[value_col].median().reset_index()
        )
    ...
    if respect_holidays:
        if not holiday_dates:
            holiday_dates = set(
                out[timestamp_col][tmp.get("_is_holiday", False)].dt.date
            )
