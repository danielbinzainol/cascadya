...
- filtrage des cartes par tags, roles ou groupes
- client OIDC `cascadya-portal-web` cree dans le realm `cascadya-corp`
- client `cascadya-portal-web` synchronise de facon persistante par le role Ansible Keycloak
...
Pour cette phase 1, on ne touche pas encore au `Control Panel`.
...
- conserver les groupes comme mecanisme optionnel d'agregation
...
Le client existe deja. La verification porte donc sur sa conformite avec la runtime du portail.
...
Pour debloquer rapidement le compte de test de `Daniel Bin Zainol`, deux options utiles :
...
Option "tout ouvert" pour un test rapide :
...
- utiliser `portal-admin` seulement pour un test "tout ouvert" ou pour les comptes d'administration
- reserver `portal-admin` aux comptes d'administration plateforme
- reserver aussi `portal-admin` au futur onglet admin du portail et aux cartes sensibles comme `Keycloak Admin`
...
- si, dans un second temps, vous voulez ouvrir les cartes a tout le monde sans donner `portal-admin`, faites-le via un groupe standard qui porte les roles metier utiles
...
- se deconnecter aussi de Keycloak si necessaire
...
1. verifier seulement le client `cascadya-portal-web`
...
2. creer ou verifier les roles realm metier
3. attribuer a `Daniel Bin Zainol` :
   - `portal-access`
   - `control-panel-user`
   - `monitoring-user`
   - `grafana-user`
4. pour un test "tout ouvert", attribuer plutot `portal-admin`
5. se deconnecter completement du portail
6. se deconnecter de Keycloak si necessaire
7. recharger `https://portal.cascadya.internal`
8. verifier que les cartes `Control Panel`, `Features`, `Grafana` et `Mimir` passent en mode ouvert

## 10. Etape suivante apres validation

Une fois les roles confirmes en prod technique :

- conserver `PORTAL_REQUIRED_TAGS=portal-access,portal-admin`
- documenter officiellement le modele `role -> cartes` dans le PRD portail
- automatiser ensuite les roles et groupes Keycloak si l'on veut rendre ce lot persistent au meme niveau que le client OIDC
- tourner ensuite le `client_secret` temporaire du portail et mettre a jour la runtime du portail avec la nouvelle valeur
