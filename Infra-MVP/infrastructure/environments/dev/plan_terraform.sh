...
if [ -n "${SCW_ACCESS_KEY:-}" ] && [ -n "${SCW_SECRET_KEY:-}" ]; then
    ...
    if [ ! -f "./secure/role_id.txt" ] || [ ! -f "./secure/secret_id.txt" ]; then
        echo "âŒ Error: role_id.txt or secret_id.txt not found in ./secure/"
        exit 1
    fi
...
echo ""
echo "âœ… Plan generated successfully. Please review the output above."
